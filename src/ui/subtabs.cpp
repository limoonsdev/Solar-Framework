...
namespace Solar::UI {
    ...
    bool Subtabs::Tab(const char* label, int id, int* currentSubTab) {
        ...
        ImVec2 size(textSize.x + 28.0f, 30.0f);
        ...
        if (anim > 0.01f) {
            ...
            draw->AddRectFilled(min, max, pal.Accent.WithAlpha(0.14f * anim).ToU32(), 5.0f);
            draw->AddRect(min, max, pal.Accent.WithAlpha(0.30f * anim).ToU32(), 5.0f, 0, 1.0f);

            // Glowing Bottom Accent Indicator Bar
            float barW = (size.x - 12.0f) * anim;
            ...
            draw->AddRectFilled(ImVec2(barMin.x, barMin.y - 1.0f), ImVec2(barMax.x, barMax.y + 1.0f),
                                pal.Accent.WithAlpha(0.35f * anim).ToU32(), 2.0f);
            draw->AddRectFilled(barMin, barMax, pal.Accent.ToU32(), 2.0f);
        } else if (hovered) {
            draw->AddRectFilled(min, max, pal.CardHover.WithAlpha(0.40f).ToU32(), 5.0f);
            draw->AddRect(min, max, IM_COL32(255, 255, 255, 14), 5.0f, 0, 1.0f);
        }

        u32 col = selected ? pal.TextPrimary.ToU32() : (hovered ? pal.TextPrimary.ToU32() : pal.TextSecondary.ToU32());
        draw->AddText(ImVec2(p.x + (size.x - textSize.x) * 0.5f, p.y + (size.y - textSize.y) * 0.5f - 1.0f), col, label);
